...
import { App, Button, Card, Col, Form, Input, InputNumber, Modal, Row, Select, Tag } from 'antd';
import React, { useEffect, useState } from 'react';
import { createPackage, deletePackage, getPackageList, updatePackage } from '@/services/saas-zero/package';
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'flex-end' }}>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditRecord(null); form.resetFields(); setModalOpen(true); }}>{f('pages.tenant.package.create')}</Button>
      </div>
      <Row gutter={[24, 24]}>
        {packages.map((pkg) => (
      ...
      <Modal
        title={f(editRecord ? 'pages.tenant.package.edit' : 'pages.tenant.package.create')}
        open={modalOpen}
        onOk={async () => {
          const values = await form.validateFields();
          if (editRecord) { await updatePackage({ ...values, id: editRecord.id }); } else { await createPackage(values); }
          message.success(f('message.' + (editRecord ? 'updateSuccess' : 'createSuccess'))); setModalOpen(false); load();
        }}
